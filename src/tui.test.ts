...
describe("scheduler TUI plugin", () => {
  test("filters status projections by query, state, and project", () => {
    const make = (name: string, health: SchedulerJobStatus["health"], scopeId: string) => ({
      name,
    ...
    const jobs = [make("Daily report", "healthy", "one"), make("Failed sync", "drifted", "one"), make("Paused backup", "paused", "two")]
